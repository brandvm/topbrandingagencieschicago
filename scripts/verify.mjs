...
for(const file of files.filter(f=>f.endsWith('.html')&&!f.startsWith('dist/design/')&&!legacyFiles.includes(f))){
 ...
 if(!staging){for(const script of doc.querySelectorAll('script:not([type="application/ld+json"])'))assert.equal(script.getAttribute('src'),base+'/_analytics/consent.js',file+' only the consent controller may execute');assert(!doc.querySelector('#site-mode'));assert(!existsSync('dist/_review'));}
 ...
 report.push({page:file.replace('dist',''),cssGzipBytes:css,fontBytes:fonts,javascriptGzipBytes:staging?files.filter(f=>f.startsWith('dist/_review/')&&f.endsWith('.js')).reduce((n,f)=>n+gzipSync(readFileSync(f)).length,0):files.filter(f=>f.startsWith('dist/_analytics/')&&f.endsWith('.js')).reduce((n,f)=>n+gzipSync(readFileSync(f)).length,0),profiles:doc.querySelectorAll('.agency-profile').length});
}
...
if(staging){
 ...
 assert.equal(designPages.length,2);
